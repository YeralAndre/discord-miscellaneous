...
export async function paginationSystem(
  ...
  if (buttons.length <= 0) throw new DsMiError('You need to specify at least 2 buttons');
  let page = 0;
  ...
  switch (type) {
    case 'REPLY':
      msg = await message.reply({
        embeds: [embeds[page].setFooter(`${messages.pages} ${page + 1} / ${embeds.length}`)],
        components: [row],
      });
      break;
    case 'SEND':
  ...
  const collector = await msg.createMessageComponentCollector({
    filter: (b) =>
      (b.customId === buttons[0].customId || b.customId === buttons[1].customId) && b.user.id === message.author.id,
  ...
  collector.on('end', () => {
    if (!msg.deleted) {
      const disRow = new MessageActionRow().addComponents(buttons[0].setDisabled(true), buttons[1].setDisabled(true));
      msg.edit({
        embeds: [embeds[page].setFooter(`${messages.pages} ${page + 1} / ${embeds.length}`)],
        components: [disRow],
      });
